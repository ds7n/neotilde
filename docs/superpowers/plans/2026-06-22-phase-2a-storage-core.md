...
Phase 0 locked the data model *schema primitives* (`Host`/`Identity`/`Defaults`/`Inherited<T>`, `RecordEnvelope` AES-256-GCM seal) but deliberately deferred the **remaining host fields** and the **storage/CRUD layer** to Phase 2 (see `docs/superpowers/plans/2026-06-17-phase-0-foundation.md:933`). The roadmap's Phase 2 (`...-glymr-implementation-roadmap.md:85`) is *Storage & sync*: Keychain identity store, `known_hosts`, CloudKit records wrapped in the AES envelope, per-category sync toggles.

The dev environment is Linux/Docker (no Apple SDKs; `swift test` runs in `glymr-dev`). The roadmap's cross-cutting constraint requires keeping the Apple-only layer thin and maximizing the Linux-testable surface. So this plan builds the **platform-agnostic storage core** — everything that compiles and is fully tested on Linux today — and defers the Apple concrete backends (Keychain/SE via `SecAccessControl`, CloudKit Private DB) to a later sub-phase (2b, macOS CI).
...
The two `protocol`s (`BlobStore`, `SecretStore`) are the only seams the Apple backends (2b) must fill. Everything above them is pure, value-type, Linux-tested Swift. Record sealing reuses the existing `RecordEnvelope` (`Sources/GlymrKit/Crypto/RecordEnvelope.swift`); cycle detection reuses the existing `hasCycle` (`Sources/GlymrKit/Model/Resolution.swift:18`).

## Tech Stack

Swift 6 (`GlymrKit`, platform-agnostic), XCTest. Crypto via `CryptoKit` on Apple / `Crypto` (swift-crypto) on Linux behind `#if canImport(CryptoKit)` (pattern already in `RecordEnvelope.swift`). Run on Linux: `docker compose run --rm dev swift test`.
...
- Placement: Swift in `Sources/GlymrKit/Model/` (schema, resolution) and a new `Sources/GlymrKit/Storage/` (stores); tests in `Tests/GlymrKitTests/`.
...
### Task 1: Complete the host schema (Tier 1 + Tier 2 + Glymr extensions)
...
**Files:**
- Create: `Sources/GlymrKit/Model/HostExtensions.swift` (nested config types + auth/mode enums)
- Modify: `Sources/GlymrKit/Model/Host.swift` (add fields to `Host` and `Defaults`)
- Test: `Tests/GlymrKitTests/HostSchemaTests.swift`
...
```swift
// Tests/GlymrKitTests/HostSchemaTests.swift
...
- [ ] **Step 5: Commit** — `feat: complete host schema (Tier 1/2 + glymr extensions)`
...
**Files:**
- Modify: `Sources/GlymrKit/Model/Resolution.swift` (keep `resolvePort`, `hasCycle`; add the rest)
- Test: `Tests/GlymrKitTests/ResolutionTests.swift`
...
```swift
// Tests/GlymrKitTests/ResolutionTests.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import XCTest
@testable import GlymrKit
...
**Files:**
- Create: `Sources/GlymrKit/Storage/BlobStore.swift`
- Test: `Tests/GlymrKitTests/InMemoryBlobStoreTests.swift`
...
```swift
// Tests/GlymrKitTests/InMemoryBlobStoreTests.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import XCTest
@testable import GlymrKit
...
A file-backed `BlobStore` for local persistence and Linux integration tests, mirroring `LearnedStore`'s atomic-write idiom (`Sources/GlymrKit/Predictor/LearnedStore.swift:45-58`). Layout: one file per record at `<dir>/<type>/<uuid>.rec`.

**Files:**
- Modify: `Sources/GlymrKit/Storage/BlobStore.swift` (append `FileBlobStore`)
- Test: `Tests/GlymrKitTests/FileBlobStoreTests.swift`
...
```swift
// Tests/GlymrKitTests/FileBlobStoreTests.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import XCTest
@testable import GlymrKit
...
**Files:**
- Create: `Sources/GlymrKit/Storage/EncryptedRecordStore.swift`
- Test: `Tests/GlymrKitTests/EncryptedRecordStoreTests.swift`

**Interfaces:**
- Consumes: `BlobStore`, `RecordEnvelope.seal/open` (`Sources/GlymrKit/Crypto/RecordEnvelope.swift`), `SymmetricKey`.
- Produces:
...
```swift
// Tests/GlymrKitTests/EncryptedRecordStoreTests.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import XCTest
import Foundation
...
**Files:**
- Create: `Sources/GlymrKit/Storage/SecretStore.swift`
- Test: `Tests/GlymrKitTests/InMemorySecretStoreTests.swift`
...
```swift
// Tests/GlymrKitTests/InMemorySecretStoreTests.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import XCTest
import Foundation
...
**Files:**
- Create: `Sources/GlymrKit/Storage/HostKeyStore.swift`
- Test: `Tests/GlymrKitTests/HostKeyStoreTests.swift`
...
```swift
// Tests/GlymrKitTests/HostKeyStoreTests.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import XCTest
@testable import GlymrKit
...
**Files:**
- Create: `Sources/GlymrKit/Storage/HostStore.swift`
- Test: `Tests/GlymrKitTests/HostStoreTests.swift`
...
```swift
// Tests/GlymrKitTests/HostStoreTests.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import XCTest
import Foundation
...
**Files:**
- Create: `Sources/GlymrKit/Storage/SyncScope.swift`
- Test: `Tests/GlymrKitTests/SyncScopeTests.swift`
...
```swift
// Tests/GlymrKitTests/SyncScopeTests.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import XCTest
@testable import GlymrKit
