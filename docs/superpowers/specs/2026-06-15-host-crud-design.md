...
| 8 | **Glymr behavior** | Collapsed | `glymr.predictor.incognito` toggle; `glymr.tmux.attemptControlMode` toggle |
...
- **Export to `~/.ssh/config`** — slug generation, label-to-alias resolution, what to emit for Glymr extensions (mosh, Tailscale, glymr.*), fingerprint comments, identity export (public-key only).
