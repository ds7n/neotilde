...
Lock the v1 SSH algorithm policy: which key-exchange, host-key, cipher, and MAC algorithms Glymr will offer, how the user can broaden the set for legacy hosts, and what the maintenance cadence is.
...
Deprecated but not catastrophically broken. Per-host toggle `glymr.allowLegacyAlgorithms` (default `false`). When toggle is on, these are added to the negotiation list for that host. No per-connect warning.
...
Cryptographically aged enough to warrant ongoing pressure to upgrade. Per-host toggle `glymr.allowDeprecatedAlgorithms` (default `false`). When toggle is on, these are added to the negotiation list. **Every connection that negotiates a Tier 3 algorithm shows a warning** (UX below).
...
A user who genuinely needs to connect to a host that offers nothing else cannot do so from Glymr. This is the floor.

## User-facing toggles

Two checkboxes in host CRUD under "Glymr behavior":
...
Both default off. Per-host. Surfaces in [[2026-06-15-host-crud-design]]'s "Glymr behavior" section between the existing `glymr.tmux.attemptControlMode` and the existing `glymr.osc52.allow` toggles.
...
In the `glymr.*` extension namespace (per [[2026-06-15-host-config-model-design]]):
...
1. **Every major OpenSSH release.** OpenSSH's default algorithm changes are a public signal — when they drop or add an algo from defaults, Glymr's lists should be re-evaluated.
2. **At every Glymr major release** *(roughly: when the version's `.0` increments)*, a pass through this spec against current Mozilla SSH guidelines and the NIST SP 800-52 / SP 800-57 family.
...
- [[2026-06-15-host-config-model-design]] — schema gains `glymr.allowLegacyAlgorithms` and `glymr.allowDeprecatedAlgorithms`, both `bool`, default `false`, in the `glymr.*` namespace.
- [[2026-06-15-host-crud-design]] — "Glymr behavior" section gains the two toggles with the caveat copy above.
...
- [[2026-06-17-host-key-trust-design]] — the algorithm Glymr accepts as a host key is now grounded by this spec's HostKey lists.
